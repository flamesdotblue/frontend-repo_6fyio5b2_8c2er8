...
const sampleProjects = [
  { id: 1, title: "AI Study Buddy", progress: 68, due: "Nov 15", members: 4 },
  { id: 2, title: "Campus Event App", progress: 42, due: "Nov 21", members: 6 },
  { id: 3, title: "Portfolio Website", progress: 90, due: "Nov 08", members: 2 },
];

export default function DashboardPreview({ onNavigate }) {
        ...
        <div className="mt-8 grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 rounded-2xl border border-black/5 dark:border-white/10 bg-white/80 dark:bg-white/5 backdrop-blur p-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => go("projects")} className="text-sm px-3 py-1.5 rounded-lg border border-black/10 dark:border-white/10 hover:bg-white/70 dark:hover:bg-white/10 transition">
                Create new
              </button>
            </div>
            <div className="mt-4 space-y-3">
              {sampleProjects.map((p) => (
                <button
                  key={p.id}
                  onClick={() => go("projects")}
                  className="w-full text-left rounded-xl border border-black/5 dark:border-white/10 bg-white/70 dark:bg-white/5 p-4 flex items-center justify-between hover:shadow"
                >
                  <div>
